...
header("Content-Disposition: attachment; filename=inspectsum".date("Ymd-His").".xls");//ganti nama sesuai keperluan
...
if($custid<>""){		
	$sqlcust=sqlsrv_query($conn,"Select ID,PartnerNumber,CompanyTitle,PartnerName,Address,City,Province,CountryID,PostalCode,PhoneNumber,FaxNumber,Email from Partners where ID='$codcust'");
	$rcust=sqlsrv_fetch_array($sqlcust);
	$sqlcon=sqlsrv_query($conn,"Select ID,CountryName from Countries where ID='$rcust[CountryID]'");
		$rcon=sqlsrv_fetch_array($sqlcon);
...
if($buyid<>""){		
	$sqlbuy=sqlsrv_query($conn,"Select ID,PartnerNumber,CompanyTitle,PartnerName,Address,City,Province,CountryID,PostalCode,PhoneNumber,FaxNumber,Email from Partners where ID='$buyer'");
	$rbuy=sqlsrv_fetch_array($sqlbuy);
	$sqlcon=sqlsrv_query($conn,"Select ID,CountryName from Countries where ID='$rbuy[CountryID]'");
		$rcon=sqlsrv_fetch_array($sqlcon);
...
if(($buyid=="") && ($custid=="")){		
	$sqlbuy=sqlsrv_query($conn,"Select ID,PartnerNumber,CompanyTitle,PartnerName,Address,City,Province,CountryID,PostalCode,PhoneNumber,FaxNumber,Email from Partners");
	$rbuy=sqlsrv_fetch_array($sqlbuy);
	$sqlcon=sqlsrv_query($conn,"Select ID,CountryName from Countries where ID='$rbuy[CountryID]'");
		$rcon=sqlsrv_fetch_array($sqlcon);
...
select
		...
		$qrypre02=sqlsrv_query($conn,$sqlpre02);
		//$rowpre02=sqlsrv_fetch_row($qrypre02);

		//--
		$c=0;
		while ($rowpre02=sqlsrv_fetch_array($qrypre02)){
...
order by a.InspectionNo desc";
	$qryInsNo=sqlsrv_query($conn,$sqlInsNo);
		$rowInsNo=sqlsrv_fetch_array($qryInsNo);
...
$qryINTI=sqlsrv_query($conn,$sqlINTI);
$rowINTI=sqlsrv_fetch_array($qryINTI);
...
//---END SQL INTI
		$qryINTIform=sqlsrv_query($conn,$sqlINTIform);
	//--
		...
		$ydGB=0;
		$ydGC=0;
		while ($rowINTIform=sqlsrv_fetch_array($qryINTIform)){
