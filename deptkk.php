...
<head>
	<meta charset="UTF-8">
	<title>Kartu Kerja di Departemen :: online system</title>
                  ...
                  <tr>
                    <td><form id="form1" name="form1" method="post" action="?">
                        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="normal9black">
                          <tr>
                            <td colspan="2" class="boldCD6">LAPORAN KARTU KERJA DI DEPARTEMEN </td>
                          </tr>
...
$count = sqlsrv_num_rows($sql);
			...
			echo "<font class='blod9black'><a href='deptkk-xls.php?dep0=$dep0&dep=$dep&ceko=$ceko'> CETAK KE EXCEL </a> <font class='blod9black'><br><br>";
...
//---SUB dep
	if ($tglDel=="0000-00-00"){
		  //----in
		  $sqlkkCEK=sqlsrv_query($conn,"select top 1 * from PCCardPosition where PCBID='$row2[PCBID]' and Status='1' and DepartmentID='$dep' order by ID DESC");	
			$inoutINCEK="";
		  while($rowkkCEK=sqlsrv_fetch_array($sqlkkCEK)){
		  		//$rowkk=sqlsrv_fetch_row($sqlkk);
			 
				$sqlkk2CEK=sqlsrv_query($conn,"select top 1 ID,convert(char(10),Dated,103) as TglIn,convert(char(10),Dated,108) as JamIn,Dated from PCCardPosition where ID='".$rowkkCEK['ID']."' and DepartmentID='$dep' order by ID DESC", array(), array("Scrollable"=>"static"));
				$rowkk2CEK=sqlsrv_fetch_array($sqlkk2CEK);
				
				//$tglINakhirCEK=$rowkk2CEK['Dated'];
			  	if($rowkk2CEK['Dated']!=""){$tglINakhirCEK=$rowkk2CEK['Dated']->format('Y-m-d H:i:s');}
		  }
	}else{
		$sqlkkCEK=sqlsrv_query($conn,"select top 1 * from PCCardPosition where PCBID='$row2[PCBID]' and Status='1' and Dated between '$tglDel' and '$tglDel2' and DepartmentID='$dep' order by ID DESC");	
			$inoutINCEK="";
		  while($rowkkCEK=sqlsrv_fetch_array($sqlkkCEK)){
		  		//$rowkk=sqlsrv_fetch_row($sqlkk);
...
if($dep<>""){
//--SUB dep 	
$sqlkkOCEK=sqlsrv_query($conn,"select top 1 * from PCCardPosition where PCBID='".$row2['PCBID']."' and Status='0' and Dated > '$tglINakhirCEK' and DepartmentID='$dep' order by ID DESC", array(), array("Scrollable"=>"static"));
			$inoutOutCEK="";
...
$sqlcek1= sqlsrv_query($conn,"select top 1 * from pccardposition where PCBID='$row[PCBID]' order by dated desc", array(), array("Scrollable"=>"static"));
...
order by a.pcbid,a.id desc";
	...
	$sqlcek2=sqlsrv_query($conn,$querycek2, array(), array("Scrollable"=>"static"));
...
//---dep
		  //----in
		  $sqlkk=sqlsrv_query($conn,"select top 1 * from PCCardPosition where PCBID='".$row2['PCBID']."' and Status='1' and DepartmentID='$dep' order by ID desc", array(), array("Scrollable"=>"static"));	
			$inoutIN="";
...
order by z.ID desc";
		  ...
		  while ($rowkk=sqlsrv_fetch_array($sqlkk)){
				...
				$sqlkk2=sqlsrv_query($conn,"select ID,convert(char(10),Dated,103) as TglIn,convert(char(10),Dated,108) as JamIn,Dated  from PCCardPosition where ID='$rowkk[ID]' and DepartmentID='$rowkk[DepartmentID]' order by Dated", array(), array("Scrollable"=>"static"));
				$rowkk2=sqlsrv_fetch_array($sqlkk2);
				$inoutIN="$inoutIN <font class='Bold333'>$rowkk2[TglIn]</font> $rowkk2[JamIn]|";
				//$tglINakhir=$rowkk2['Dated'];
			  	$tglINakhir=$rowkk2['Dated']->format('Y-m-d H:i:s');
...
if($dep<>""){
//--dep 	
$sqlkkO=sqlsrv_query($conn,"select top 1 * from PCCardPosition where PCBID='".$row2['PCBID']."' and Status='0' and Dated >= '$tglINakhir' and DepartmentID='$dep' order by ID desc", array(), array("Scrollable"=>"static"));
			$inoutOut="";
