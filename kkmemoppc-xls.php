...
if($custid<>""){		
	$sqlcust=sqlsrv_query($conn1,"Select ID,PartnerNumber,CompanyTitle,PartnerName,Address,City,Province,CountryID,PostalCode,PhoneNumber,FaxNumber,Email from Partners where ID='$codcust'", array(), array("Scrollable"=>"buffered"));
	$rcust=sqlsrv_fetch_array($sqlcust,SQLSRV_FETCH_ASSOC);
	$sqlcon=sqlsrv_query($conn1,"Select ID,CountryName from Countries where ID='$rcust[CountryID]'");
...
if($buyid<>""){		
	$sqlbuy=sqlsrv_query($conn1,"Select ID,PartnerNumber,CompanyTitle,PartnerName,Address,City,Province,CountryID,PostalCode,PhoneNumber,FaxNumber,Email from Partners where ID='$buyer'", array(), array("Scrollable"=>"buffered"));
	$rbuy=sqlsrv_fetch_array($sqlbuy,SQLSRV_FETCH_ASSOC);
	$sqlcon=sqlsrv_query($conn1,"Select ID,CountryName from Countries where ID='$rbuy[CountryID]'", array(), array("Scrollable"=>"buffered"));
		$rcon=sqlsrv_fetch_array($sqlcon,SQLSRV_FETCH_ASSOC);
...
$sqlb = sqlsrv_query($conn1,$sql0b, array(), array("Scrollable"=>"buffered")) 
    or die('A error occured :0C');
 
$count = sqlsrv_fetch_array($sqlb,SQLSRV_FETCH_ASSOC);
...
echo "</table>";
     ...
     echo " <table width='100%' border='1'>";
	  ...
	  //--150605 		echo "<td class='tombol'><div align='center'>Posisi Sebelumnya </div></td>";
		  ...
		  //QCF		
		  echo "   <td class='tombol'><div align='center'>Ket. TK </div></td>";		
...
where $filterbuy and $filtertgl and pcb.Gross<>'0' order by jo.DocumentNo,pm.Description";
		$qrypre02=sqlsrv_query($conn1,$sqlpre02, array(), array("Scrollable"=>"buffered"));
		...
		//-------begin		
		$sqlpre2=sqlsrv_query($conn1,"select top 1 ID,Dated,PCBID,DepartmentID from PCCardPosition where PCBID='$rowpre02[PCBID]' order by ID desc", array(), array("Scrollable"=>"buffered"));
...
$sql2b = sqlsrv_query($conn1,$sql2, array(), array("Scrollable"=>"buffered")) 
...
$row2=sqlsrv_fetch_array($sql2b,SQLSRV_FETCH_ASSOC);
		...
		//----cari salinan
			   $child=$row2['ChildLevel'];
					...
					if($child > 0){
						$sqlgetparent=sqlsrv_query($conn1,"select ID,LotNo from ProcessControlBatches where ID='$row2[RootID]' and ChildLevel='0'", array(), array("Scrollable"=>"buffered"));
			...
			$sqlkk=sqlsrv_query($conn1,"select top 2 * from PCCardPosition where PCBID='$row2[PCBID]' and CounterDepartmentID<>'$row2[DepartmentID]' order by Dated desc", array(), array("Scrollable"=>"buffered"));
			//$rowkk=sqlsrv_num_rows($sqlkk);
	...
	if ($Muncul=="YA"){
		...
		//----
	//	if($buyid<>""){	
		$sqlgetcust=sqlsrv_query($conn1,"Select ID,PartnerNumber from Partners where ID='$row2[CustomerID]'", array(), array("Scrollable"=>"buffered"));
	...
	//	 echo "<td class='normal333' valign=top>$row2[TglSO]</td>";
          ...
          //--lot
					  ...
					  $qryLot = sqlsrv_query($conn1,$sqlLot, array(), array("Scrollable"=>"buffered")) 
		 ...
		 echo "<td width='80' class='normal333' valign=top align=center>$row2[RollCount]</td>";
		   echo "<td width='80' class='normal333' valign=top>" .number_format($row2['Weight'],'2','.',','). "</td>";
			...
			//---POSISI SEBELUMnya
				...
				$sqlpsb=sqlsrv_query($conn1,"Select top 2 * from PCCardPosition where PCBID='$row2[PCBID]' and Status ='0' and DepartmentID<>'$row2[DepartmentID]' order by Dated desc", array(), array("Scrollable"=>"buffered"));
				$rowpsb=sqlsrv_num_rows($sqlpsb);
				
				$sqlkkpsb=sqlsrv_query($conn1,"select pcpos.*,dep.DepartmentName as DepOut,convert(char(10),Dated,103) as TglIn,convert(char(10),Dated,108) as JamIn  from PCCardPosition pcpos 
				inner join
	...
	//--150605	  	echo "<td width='120' class='BoldCD6' valign='top'><font class='normal7black'>$rowkkpsb[DepOut]  <br>Out: $rowkkpsb[TglIn] $rowkkpsb[JamIn]</font></td>";
		  ...
		  //--		  
		  //--Kode Dept
			$DepKode=sqlsrv_query($conn1,"Select ParentID,ID from Departments where ID='$row2[deptID]'", array(), array("Scrollable"=>"buffered"));
			//$RKodeDep=sqlsrv_num_rows($DepKode);
		  ...
		  if ($rowkk['Status']==0){ // =0 : out

		  
				$sqlkk2=sqlsrv_query($conn1,"select pcpos.*,dep.DepartmentName as DepOut,convert(char(10),Dated,103) as TglIn,convert(char(10),Dated,108) as JamIn  from PCCardPosition pcpos 
				inner join
				Departments dep on pcpos.CounterDepartmentID  = dep.ID
				where pcpos.ID='$rowkk[ID]' order by Dated desc", array(), array("Scrollable"=>"buffered"));
		  ...
		  }elseif ($rowkk['Status']<>0){ // =1 : in
		  	$sqlkk2=sqlsrv_query($conn1,"select ID,convert(char(10),Dated,103) as TglIn,convert(char(10),Dated,108) as JamIn,DepartmentID  from PCCardPosition where ID='$rowkk[ID]' order by Dated desc", array(), array("Scrollable"=>"buffered"));
			$rowkk2=sqlsrv_fetch_array($sqlkk2,SQLSRV_FETCH_ASSOC);
			//--warning	
		...
		//---setting
		  $sqlset=sqlsrv_query($conn1,"select ID,DocumentNo from ProcessControlBatches where DocumentNo='$row2[NoKK]'", array(), array("Scrollable"=>"buffered"));
		  $cset=sqlsrv_num_rows($sqlset);
		  if ($cset > 0 ){
		  	$rowset=sqlsrv_fetch_array($sqlset,SQLSRV_FETCH_ASSOC);
				$sqlflow=sqlsrv_query($conn1,"select top 1 *,convert(char(10),Dated,103) as TglF,convert(char(10),Dated,108) as JamF from ProcessFlowProcessNo where ParentID='$rowset[ID]' and MachineType='14' order by Dated desc", array(), array("Scrollable"=>"buffered"));
	...
	order by 
		md.Description";
						...
						$qrymsn=sqlsrv_query($conn1,$sqlmsn, array(), array("Scrollable"=>"buffered"));
      ...
      //--150605    echo "<td width='120' class='normal333' valign=top>$row2[ProductNumber]<hr><font size=0.1>$ketsetting</font></td>";
		  ...
		  //---Dept Note
		  $sqlcarinotePCB=sqlsrv_query($conn1,"select ID,DocumentNo from ProcessControlBatches where DocumentNo='".$row2['NoKK']."'", array(), array("Scrollable"=>"static"));
		  
		  $rowcarinotePCB=sqlsrv_fetch_array($sqlcarinotePCB,SQLSRV_FETCH_ASSOC);
		  
		  $sqlcarinotePFPN=sqlsrv_query($conn1,"select top 1 ID,ParentID,Dated from ProcessFlowProcessNo where ParentID='".$rowcarinotePCB['ID']."' order by Dated desc", array(), array("Scrollable"=>"static"));
		  
		  $rowcarinotePFPN=sqlsrv_fetch_array($sqlcarinotePFPN,SQLSRV_FETCH_ASSOC);
		  
		  $sqlcarinotePFDN=sqlsrv_query($conn1,"select ParentID,DepartmentID,cast(Note as nvarchar(200)) as Note  from ProcessFlowDetailsDeptNote where DepartmentID='".$row2['deptID']."' and ParentID ='".$rowcarinotePFPN['ID']."'", array(), array("Scrollable"=>"static"));
		  
		  $rowcarinotePFDN=sqlsrv_fetch_array($sqlcarinotePFDN,SQLSRV_FETCH_ASSOC);
		  
		  //---note begin
		  $sqlnote0=sqlsrv_query($conn1,"Select top 1 id,parentid,machinetype from Processflowprocessno where parentid='".$rowcarinotePCB['ID']."' and machinetype='24' order by id desc", array(), array("Scrollable"=>"static"));
		  $rownote0=sqlsrv_fetch_array($sqlnote0,SQLSRV_FETCH_ASSOC);
		  
		  $sqlPFnote=sqlsrv_query($conn1,"select top 1 ParentID,cast(Note as nvarchar(200)) as Note from ProcessFlowDetailsNote where ParentID ='".$rownote0['id']."' order by entrytype desc", array(), array("Scrollable"=>"static"));
		  ...
		  $jm=($posisiQCF1)-($posisiQCF+3);		  
		  $potongQCF=substr($catatan,$posisiQCF+3,$jm);	
		  $catatQCF=$potongQCF;   
		  }
		  ...
		  //------tambahan kolom
			...
			//---
		  //---Dept Note
		  $sqlcarinotePCB=sqlsrv_query($conn1,"select ID,DocumentNo from ProcessControlBatches where DocumentNo='$row2[NoKK]'", array(), array("Scrollable"=>"buffered"));
		  
		  $rowcarinotePCB=sqlsrv_fetch_array($sqlcarinotePCB,SQLSRV_FETCH_ASSOC);
		  
		  $sqlcarinotePFPN=sqlsrv_query($conn1,"select top 1 ID,ParentID,Dated from ProcessFlowProcessNo where ParentID='$rowcarinotePCB[ID]' order by Dated desc", array(), array("Scrollable"=>"buffered"));
		  
		  $rowcarinotePFPN=sqlsrv_fetch_array($sqlcarinotePFPN,SQLSRV_FETCH_ASSOC);
		  
		  $sqlcarinotePFDN=sqlsrv_query($conn1,"select ParentID,DepartmentID,cast(Note as nvarchar(200)) as Cat  from ProcessFlowDetailsDeptNote where DepartmentID='$row2[deptID]' and ParentID ='$rowcarinotePFPN[ID]'", array(), array("Scrollable"=>"buffered"));
