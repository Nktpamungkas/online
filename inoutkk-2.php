...
$row2=mssql_fetch_assoc($sql2b);
		...
		echo "<td width='120' class='normal333'  valign=top>$row2[DocumentNo]</td>";
		  ...
		  //echo "tess :$rowkk[5]";
		  //----in
		  $sqlkk=mssql_query("select * from PCCardPosition where PCBID='$row2[PCBID]' and Status='1' and Dated between '$tglDel' and '$tglDel2' order by Dated");	
			$inoutIN="";
		  while ($rowkk=mssql_fetch_assoc($sqlkk)){
		  		//$rowkk=mssql_fetch_row($sqlkk);
			 
				$sqlkk2=mssql_query("select ID,convert(char(10),Dated,103) as TglIn,convert(char(10),Dated,108) as JamIn,Dated  from PCCardPosition where ID='$rowkk[ID]' order by Dated");
		  ...
		  //--Out
		//$sqlkkO=mssql_query("select * from PCCardPosition where PCBID='$row2[PCBID]' and Status='0' and Dated >= '$tglDel' order by Dated DESC");
$sqlkkO=mssql_query("select top 1 * from PCCardPosition where PCBID='$row2[PCBID]' and Status='0' and Dated > '$tglINakhir' order by Dated");
			$inoutOut="";
	...
	where pcpos.ID='$rowkkO[ID]' order by Dated desc");
			  ...
			  $InoutOUT="<font class='Bold333'>$rowkk2b[TglIn]</font> $rowkk2b[JamIn]"; // to $rowkk2b[DepOut] |";
          ...
          echo "<td width='80' class='normal333' valign=top>" .number_format($row2[Quantity],2). " $row2[UnitName]</td>";
		   echo "<td width='80' class='normal333' valign=top>" .number_format($row2[Bruto],2). " $row2[UnitName]</td>";
          echo "<td width='120' class='normal333' valign=top>$row2[ProductNumber]</td>";
          echo "<td class='normal333' valign=top>$row2[ProductDesc]</td>";
		  echo "<td width='120' class='normal333' valign=top>$row2[NoKK]</td>";
        echo "</tr>";
