...
$sql0="select
		...
		from
			...
			where soda.GrossWeight > '1000' and so.SODate between '$tgl1' and '$tgl2'
				group by
...
$sql0="select
		...
		from
			...
			from
				...
				SODetails sod on so.ID = sod.SOID inner join
				SODetailsAdditional soda on sod.ID = soda.SODID 			
			where soda.GrossWeight between '1' and '24' and so.SODate between '$tgl1' and '$tgl2'
				group by
					so.SONumber, so.SODate, so.CustomerID, so.BuyerID, so.PONumber, so.PODate,jo.DocumentNo,
					sod.ID, sod.ProductID, sod.Quantity, sod.UnitID, sod.Weight, sod.WeightUnitID,
					soda.RefNo,
...
$sql0="select
		...
		from
			...
			where soda.GrossWeight > '1' and so.SODate between '$tgl1' and '$tgl2'
				group by
					so.SONumber, so.SODate, so.CustomerID, so.BuyerID, so.PONumber, so.PODate,jo.DocumentNo,
					sod.ID, sod.ProductID, sod.Quantity, sod.UnitID, sod.Weight, sod.WeightUnitID,
					soda.RefNo,
...
$sql0="select
		...
		from
			...
			from
				...
				SODetails sod on so.ID = sod.SOID inner join
				SODetailsAdditional soda on sod.ID = soda.SODID 			
			where soda.GrossWeight between '$r1' and '$r2' and so.SODate between '$tgl1' and '$tgl2'
				group by
					so.SONumber, so.SODate, so.CustomerID, so.BuyerID, so.PONumber, so.PODate,jo.DocumentNo,
					sod.ID, sod.ProductID, sod.Quantity, sod.UnitID, sod.Weight, sod.WeightUnitID,
					soda.RefNo,
