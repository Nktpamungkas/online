      ...
      echo "<td class='tombol'><div align='center'>No LOT</div></td>";
		...
		from
			...
			from
				...
				ProcessFlowProcessNo pfpn on pfpn.EntryType = 2 and pcb.ID = pfpn.ParentID and pfpn.MachineType = 24 left join
				ProcessFlowDetailsNote pfdn on pfpn.EntryType = pfdn.EntryType and pfpn.ID = pfdn.ParentID
			where jo.DocumentNo='$nobo' and pcb.Gross<>'0' and pcb.DocumentNo='$nokk'
				group by
					so.SONumber, so.SODate, so.CustomerID, so.BuyerID, so.PONumber, so.PODate,jo.DocumentNo,
					sod.ID, sod.ProductID, sod.Quantity, sod.UnitID, sod.Weight, sod.WeightUnitID,
	...
	//	  if ($rowkk1[PCBID]==$rowkk2[ID]){
		...
		//  }
		  //--
			...
			echo "<td width='120' class='BoldCD6' valign='top'><font class='blod9black'>$row2[DepartmentName]</font><font size=0.1><br>$ketsetting</font><hr><font class='normal7black'>In: $rowkk2[TglIn] $rowkk2[JamIn] $cekDep</font></td>";
		  }
		  //--
	...
	order by 
		md.Description";
		  ...
		  echo "<td width='80' class='normal333' valign=top align=center>$row2[RollCount]</td>";
		   echo "<td width='80' class='normal333' valign=top>" .number_format($row2[Weight],2). " Kg<br>";
